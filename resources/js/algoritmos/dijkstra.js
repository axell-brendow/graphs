 ...
 */
async function percorrerVerticesNaoVisitados(
    vertices,
    listaDeIteracao,
    funcaoDeManipulacao,
    delay = 0
) {
    for (let i = 0; i < listaDeIteracao.length; i++) {
 ...
 */
async function obterVerticeMaisProximo(vertices, caminhos) {
    let indice = -1;
    let menorPeso = Infinity;

    await percorrerVerticesNaoVisitados(vertices, caminhos, (i, caminho) => {
...
function inicializarDijkstra(grafo, indiceVerticeInicial) {
    ...
    let caminhos = arestasVerticeInicial.map((aresta, indice) => {
...
function nucleoDijkstra(
    ...
    if (pesoDoInicialAoMaisProx + aresta.peso < pesoDoInicialAoFinal) {
        ...
        caminhos[indiceVerticeFinal] = caminhoNovo.adicionar(
            indiceVerticeFinal
        );
 ...
 */
async function percorrerArestasEReiniciarProcesso(
    indVerticeMaisProximo,
    grafo,
    copiaCorBorda,
    copiaTamanhoBorda,
    ...
    await percorrerVerticesNaoVisitados(
        vertices,
        arestasDoVertice,
        (indice, aresta) => {
    ...
    const indice = await obterVerticeMaisProximo(vertices, caminhos);
...
async function dijkstra(
    ...
    while (
        indVerticeMaisProximo != -1 &&
        indVerticeMaisProximo != indiceVerticeFinal
    ) {
        const valores = await percorrerArestasEReiniciarProcesso(
            indVerticeMaisProximo,
            grafo,
            copiaCorBorda,
            copiaTamanhoBorda,
    ...
    const indice = await obterVerticeMaisProximo(vertices, caminhos);
    indVerticeMaisProximo = indice;
    return (dijkstraCaminhos = caminhos);
}
