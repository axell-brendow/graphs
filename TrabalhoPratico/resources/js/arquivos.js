...
function carregarArquivo(arquivo, funcaoParaManipular) {
    ...
    return new Promise((resolve, reject) => {
        leitor.onload = (
            event // É disparado quando o métodos de read é chamado
        ) => {
...
function carregarGrafoDasDistancias(vertices) {
    ...
    percorrerMetadeDaMatriz(grafoDistancias, (linha, coluna, aresta) => {
...
function obterAbscissa(longitude, larguraMaxima) {
    ...
    let abscissa =
        0.00284141 * larguraMaxima * longitude + 0.465705 * larguraMaxima;
...
function obterOrdenada(latitude, alturaMaxima) {
    ...
    let ordenada =
        -0.00659341 * alturaMaxima * latitude + 0.628132 * alturaMaxima;
 ...
 */
async function carregarAeroportos(arquivo) {
    let sigla, latitude, longitude;
    dictSiglas = {};
    vertices = [];

    const numeroDeEntradas = await carregarArquivo(
        arquivo,
        (colunas, indiceLinha) => {
            sigla = colunas[0];
 ...
 */
async function carregarPrecos(arquivo) {
    if (numeroDeVertices <= 0) {
        ...
        await carregarArquivo(arquivo, (colunas, indiceLinha) => {
            ...
            arestaOrigemDestino.peso = arestaDestinoOrigem.peso = parseInt(
                preco
            );
